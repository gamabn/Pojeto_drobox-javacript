...
class DropBoxController{
    ...
    // this.progess = document.querySelector('mc-progress-bar-bg');
    ...
    connectFirebase(){
        var config = {
            apiKey: "AIzaSyDOnFkp1yWcaeUUWp1-3mpIW2pCaQ9TKEc",
            authDomain: "dropbox-clone-38d8b.firebaseapp.com",
            databaseURL: "https://dropbox-clone-38d8b-default-rtdb.firebaseio.com",
            projectId: "dropbox-clone-38d8b",
            storageBucket: "dropbox-clone-38d8b.appspot.com",
            messagingSenderId: "607546969502",
            appId: "1:607546969502:web:a19dc23791b4ca5293da8f",
            measurementId: "G-5QRBQJRPFT"
        };
   ...
   // console.log(file);
    ...
    initEvents() {
      this.btnDelete.addEventListener("click", (e) => {//
        this.removeTask().then((response)=>{
          ...
          response.forEach(snapshotkey => {
            if ( snapshotkey && snapshotkey) {
              console.log(snapshotkey.path);

              firebase.database().ref('files').FieldValue.delete(snapshotkey.key)
              //this.getFirebaseRef().doc(snapshotkey).delete()

             // this.getFirebaseRef().child(snapshotkey.fields.filepath).remove()
    ...
    uploadProgress(event,file){
     ...
     //this.progess.style.width = `${porcent}%`;
     this.namefileEl.innerHTML = file.name; 
     this.timeleftEl.innerHTML = this.formtTimeToHuman(timespent);
    ...
    getFileIconView(file) {
     console.log(file[0].mimetype)
      ...
      getFileView(file, key) {
        let  documents = file[0].originalFilename
        let li = document.createElement('li')
        //<div class="name text-center">${documents}</div>
